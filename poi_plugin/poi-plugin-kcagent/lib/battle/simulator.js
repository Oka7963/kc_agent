...
exports["default"] = exports.BattleRankMap = exports.SupportTypeMap = exports.EngagementMap = exports.FormationMap = exports.DetectionMap = exports.AirControlMap = exports.NightAttackTypeMap = exports.DayAttackTypeMap = exports.Rank = exports.Result = exports.Detection = exports.Formation = exports.Engagement = exports.EngagementInfo = exports.AirControl = exports.AerialInfo = exports.ShipOwner = exports.Ship = exports.HitType = exports.MultiTargetAttackOrder = exports.MultiTargetAttackType = exports.AttackType = exports.Attack = exports.StageType = exports.Stage = void 0;
...
const StageType = {
  ...
  // Shelling
  Support: "Support",
  // Support Fleet (Expedition)
  LandBase: "LandBase",
  // Land Base Aerial Support
  Engagement: "Engagement",
  // [SP] Engagement Information
  // Sub Type
  Main: "Main",
  // Shelling, main fleet
  Escort: "Escort",
  // Shelling, escort fleet
  Night: "Night",
  // Shelling, night combat
  Opening: "Opening",
  // Torpedo,  opening torpedo salvo
...
const AttackType = {
  ...
  // 	Colorado戦隊、全力斉射！各個目標に砲撃開始！一気に殲滅する、Fire!!
...
const MultiTargetAttackType = new Set([AttackType.Nelson_Touch, AttackType.Nagato_Punch, AttackType.Mutsu_Splash, AttackType.Colorado_Fire, AttackType.Kongo_Class_Kaini_C_Charge, AttackType.Yamato_Attack_Double, AttackType.Yamato_Attack_Triple, AttackType.Baguette_Charge, AttackType.QE_Touch, AttackType.Submarine_Special_Attack_2_3, AttackType.Submarine_Special_Attack_3_4, AttackType.Submarine_Special_Attack_2_4, AttackType.Zuiun_Night_Attack, AttackType.Laser, AttackType.Type_4_LC_Special_Attack]);
exports.MultiTargetAttackType = MultiTargetAttackType;
const MultiTargetAttackOrder = {
...
exports.MultiTargetAttackOrder = MultiTargetAttackOrder;
const HitType = {
  Miss: 0,
  Hit: 1,
  Critical: 2
...
class Ship {
  constructor(opts) {
    ...
    this.finalParam = opts.finalParam; // parameter = [FRP, TORP, AA, AMOR]
...
exports.DayAttackTypeMap = DayAttackTypeMap;
...
exports.NightAttackTypeMap = NightAttackTypeMap;
const AirControlMap = {
  0: AirControl.Parity,
...
exports.AirControlMap = AirControlMap;
const DetectionMap = {
  1: Detection.Success,
...
exports.DetectionMap = DetectionMap;
const FormationMap = {
  1: Formation.Ahead,
  2: Formation.Double,
  3: Formation.Diamond,
...
exports.FormationMap = FormationMap;
const EngagementMap = {
...
exports.SupportTypeMap = SupportTypeMap;
const BattleRankMap = {
  'SS': Rank.SS,
  'S': Rank.S,
  'A': Rank.A,
...
exports.BattleRankMap = BattleRankMap;
const HalfSunkNumber = [// 7~12 is guessed.
0, 1, 1, 2, 2, 3, 4, 4, 5, 6, 7, 7, 8];

function useItem(ship) {
  if (ship.owner === ShipOwner.Ours && ship.nowHP <= 0 && ship.items != null) for (const itemId of ship.items) {
    // 応急修理要員
    if (itemId === 42) {
...
function damageShip(fromShip, toShip, damage) {
  ...
  let toHP = toShip.nowHP; // `toShip.*` is updated in place
...
function generateEngagementInfo(packet, oursFleet, emenyFleet, opts = {}) {
  ...
  if (opts.engagement) {
    ...
    o.weakened = [api_boss_damaged, api_xal01].find(x => x != null); // Carry over smoke_type

    const {
      api_smoke_type
    } = packet; // old records doesn't have this key, unifying to 0.
...
function simulateAerialAttack(fleet, edam, ebak_flag, erai_flag, ecl_flag) {
  ...
  for (let [i, damage] of edam.entries()) {
    if (damage < 0 || ebak_flag[i] <= 0 && erai_flag[i] <= 0) continue;
...
function simulateTorpedoAttack(mainFleet, escortFleet, enemyFleet, enemyEscort, eydam, erai, ecl) {
  ...
  for (let [i, t] of erai.entries()) {
    ...
    if (i < 0 || t < 0) continue;
    if (i < mainFleetRange) fromShip = mainFleet[i];else fromShip = escortFleet[i - mainFleetRange];
    if (t < enemyFleetRange) toShip = enemyFleet[t];else toShip = enemyEscort[t - enemyFleetRange];
    ...
    let {
      ...
      toHP,
      item
    } = damageShip(fromShip, toShip, damage);
    list.push(new Attack({
      type: AttackType.Normal,
      fromShip: fromShip,
      toShip: toShip,
...
function simulateTorpedo(mainFleet, escortFleet, enemyFleet, enemyEscort, raigeki, subtype) {
  ...
  let attacks = [];
  if (raigeki.api_frai != null) attacks = attacks.concat(simulateTorpedoAttack(mainFleet, escortFleet, enemyFleet, enemyEscort, raigeki.api_fydam, raigeki.api_frai, raigeki.api_fcl));
  if (raigeki.api_erai != null) attacks = attacks.concat(simulateTorpedoAttack(enemyFleet, enemyEscort, mainFleet, escortFleet, raigeki.api_eydam, raigeki.api_erai, raigeki.api_ecl));
  return new Stage({
...
function simulateOpeningTorpedo(mainFleet, escortFleet, enemyFleet, enemyEscort, raigeki, subtype) {
  ...
  let attacks = [];
  if (raigeki.api_frai_list_items != null) attacks = attacks.concat(simulateOpeningTorpedoAttack(mainFleet, escortFleet, enemyFleet, enemyEscort, raigeki.api_fydam_list_items, raigeki.api_frai_list_items, raigeki.api_fcl_list_items));
  if (raigeki.api_erai_list_items != null) attacks = attacks.concat(simulateOpeningTorpedoAttack(enemyFleet, enemyEscort, mainFleet, escortFleet, raigeki.api_eydam_list_items, raigeki.api_erai_list_items, raigeki.api_ecl_list_items));
  return new Stage({
    type: StageType.Torpedo,
    attacks: attacks,
    subtype: subtype
...
function simulateShelling(mainFleet, escortFleet, enemyFleet, enemyEscort, hougeki, subtype) {
  ...
  for (let [i, attacker] of (hougeki.api_at_list || []).entries()) {
    ...
    if (!MultiTargetAttackType.has(attackType)) {
      ...
      for (const cl of hougeki.api_cl_list[i]) hit.push(cl === 2 ? HitType.Critical : cl === 1 ? HitType.Hit : HitType.Miss);

      let {
        fromHP,
        toHP,
        item
      } = damageShip(fromShip, toShip, damageTotal);
      list.push(new Attack({
        type: attackType,
        fromShip: fromShip,
        toShip: toShip,
      ...
      for (let j = 0; j < hougeki.api_df_list[i].length; j++) {
        ...
        let {
          ...
          toHP,
          item
        } = damageShip(fromShip, toShip, dmg);
        list.push(new Attack({
          type: attackType,
          fromShip: fromShip,
          toShip: toShip,
...
function simulateSupport(enemyFleet, enemyEscort, support, flag) {
  ...
  if (flag === 2 || flag === 3) {
    ...
    for (let [i, damage] of hourai.api_damage.entries()) {
      let toShip;
      if (0 <= i && i < enemyFleetRange) toShip = enemyFleet[i];
      ...
      let hit = cl === 2 ? HitType.Critical : cl === 1 ? HitType.Hit : HitType.Miss; // No showing Miss attack on support stage.
...
function simulateBattleRank(mainFleet, escortFleet, enemyFleet, enemyEscort) {
  ...
  if (ours.sunk === 0) {
    if (enemy.sunk === enemy.num) {
      if (ours.lostHP <= 0) return Rank.SS;else return Rank.S;
    }
...
function simulateFleetNightMVP(stages) {
  ...
  for (const stage of stages) {
    ...
    for (const attack of stage.attacks) {
      ...
      if (6 <= pos && pos <= 11) sum[pos - 7] += damage.reduce((x, y) => x + y, 0);else console.warn("Non-escort fleet ship attack in night stage", ship, attack);
    }
  }
...
class Simulator2 {
  ...
  static auto(battle, opts) {
    ...
    for (const packet of battle.packet) s.simulate(packet);
      ...
      if (rawShip != null) {
        ...
        if (this.usePoiAPI) {
          ...
          if (typeof $ship != 'undefined') {
            baseParam = [$ship.api_houg[0] + kyouka[0], $ship.api_raig[0] + kyouka[1], $ship.api_tyku[0] + kyouka[2], $ship.api_souk[0] + kyouka[3]];
          }

          finalParam = [rawShip.api_karyoku[0], rawShip.api_raisou[0], rawShip.api_taiku[0], rawShip.api_soukou[0]];
        }
      ...
      if (typeof id === "number" && id > 0) {
        if (this.usePoiAPI) {
          ...
          finalParam = slots.reduce((bonus, id) => {
            ...
            return [bonus[0] + (item.api_houg || 0), bonus[1] + (item.api_raig || 0), bonus[2] + (item.api_tyku || 0), bonus[3] + (item.api_souk || 0)];
          }, baseParam);
        }
  ...
  simulate(packet) {
    ...
    this.assert(packet, path); // Engagement
    ...
    if (['/kcsapi/api_req_practice/battle', '/kcsapi/api_req_sortie/battle', '/kcsapi/api_req_sortie/airbattle', '/kcsapi/api_req_sortie/ld_airbattle', '/kcsapi/api_req_sortie/ld_shooting', '/kcsapi/api_req_combined_battle/battle', '/kcsapi/api_req_combined_battle/battle_water', '/kcsapi/api_req_combined_battle/airbattle', '/kcsapi/api_req_combined_battle/ld_airbattle', '/kcsapi/api_req_combined_battle/ld_shooting', '/kcsapi/api_req_combined_battle/ec_battle', '/kcsapi/api_req_combined_battle/each_battle', '/kcsapi/api_req_combined_battle/each_battle_water'].includes(path)) {
      this.prcsDay(packet, path);
    } // Night Battle


    if (['/kcsapi/api_req_practice/midnight_battle', '/kcsapi/api_req_battle_midnight/battle', '/kcsapi/api_req_battle_midnight/sp_midnight', '/kcsapi/api_req_combined_battle/midnight_battle', '/kcsapi/api_req_combined_battle/sp_midnight', '/kcsapi/api_req_combined_battle/ec_midnight_battle', '!COMPAT/midnight_battle'].includes(path)) {
    ...
    if (['/kcsapi/api_req_practice/battle_result', '/kcsapi/api_req_sortie/battleresult', '/kcsapi/api_req_combined_battle/battleresult'].includes(path)) {
  ...
  prepare(packet, path) {
    ...
    if (enemyFleet == null) {
      ...
      if (packet.api_ship_ke) this.enemyEscort = this._initEnemy(packet.api_ship_ke.length, packet.api_ship_ke_combined, packet.api_eSlot_combined, packet.api_e_maxhps_combined, packet.api_e_nowhps_combined, packet.api_ship_lv_combined, packet.api_eParam_combined);
    ...
    if (packet.api_friendly_info != null) {
      ...
      this.friendFleet = this._initEnemy(0, info.api_ship_id, info.api_Slot, info.api_maxhps, info.api_nowhps, info.api_ship_lv, info.api_Param, ShipOwner.Friend);
    ...
    if (['/kcsapi/api_req_sortie/ld_airbattle', '/kcsapi/api_req_sortie/ld_shooting', '/kcsapi/api_req_combined_battle/ld_airbattle', '/kcsapi/api_req_combined_battle/ld_shooting'].includes(path)) {
  ...
  assert(packet, path) {
    ...
    if (['/kcsapi/api_req_practice/battle', '/kcsapi/api_req_sortie/battle', '/kcsapi/api_req_sortie/airbattle', '/kcsapi/api_req_sortie/ld_airbattle', '/kcsapi/api_req_sortie/ld_shooting', '/kcsapi/api_req_combined_battle/ec_battle', '/kcsapi/api_req_practice/midnight_battle', '/kcsapi/api_req_battle_midnight/battle', '/kcsapi/api_req_battle_midnight/sp_midnight'].includes(path)) {
      if (!(fleetType === 0)) {
    ...
    if (['/kcsapi/api_req_combined_battle/battle', '/kcsapi/api_req_combined_battle/each_battle'].includes(path)) {
      if (!(fleetType === 1 || fleetType === 3)) {
        console.warn(`${path} expect fleet.type=1,3, but got ${fleetType}.`); // HACK: Currently CTF & TE act the same
    ...
    if (['/kcsapi/api_req_combined_battle/battle_water', '/kcsapi/api_req_combined_battle/each_battle_water'].includes(path)) {
      if (!(fleetType === 2)) {
    ...
    if (['/kcsapi/api_req_combined_battle/airbattle', '/kcsapi/api_req_combined_battle/ld_airbattle', '/kcsapi/api_req_combined_battle/ld_shooting', '/kcsapi/api_req_combined_battle/midnight_battle', '/kcsapi/api_req_combined_battle/sp_midnight'].includes(path)) {
      if (!(fleetType === 1 || fleetType === 2 || fleetType === 3)) {
        console.warn(`${path} expect fleet.type=1,2,3, but got ${fleetType}.`); // We can't detemine fleet.type
      }
    } // Any


    if (['/kcsapi/api_req_combined_battle/ec_midnight_battle'].includes(path)) {
      if (!(fleetType === 0 || fleetType === 1 || fleetType === 2 || fleetType === 3)) {
        console.warn(`${path} expect fleet.type=0,1,2,3, but got ${fleetType}.`); // We can't detemine fleet.type
      }
    }
  }
  ...
  prcsDay(packet, path) {
    ...
    const {
      stages
    } = this; // Land base air attack (assault)

    stages.push(simulateLandBase(enemyFleet, enemyEscort, packet.api_air_base_injection, true)); // Aerial Combat (assault)

    stages.push(simulateAerial(mainFleet, escortFleet, enemyFleet, enemyEscort, packet.api_injection_kouku, true)); // Land base air attack

    for (const api_kouku of packet.api_air_base_attack || []) stages.push(simulateLandBase(enemyFleet, enemyEscort, api_kouku)); // Arrial Combat (friendly)


    stages.push(simulateAerial(friendFleet, null, enemyFleet, enemyEscort, packet.api_friendly_kouku)); // Aerial Combat

    stages.push(simulateAerial(mainFleet, escortFleet, enemyFleet, enemyEscort, packet.api_kouku)); // Aerial Combat 2nd

    stages.push(simulateAerial(mainFleet, escortFleet, enemyFleet, enemyEscort, packet.api_kouku2)); // Expedition Support Fire

    stages.push(simulateSupport(enemyFleet, enemyEscort, packet.api_support_info, packet.api_support_flag)); // Normal Fleet

    if (fleetType === 0) {
      if (enemyType === 0) {
        // Opening Anti-Sub
        stages.push(simulateShelling(mainFleet, escortFleet, enemyFleet, enemyEscort, packet.api_opening_taisen, StageType.Opening)); // Opening Torpedo Salvo

        stages.push(simulateOpeningTorpedo(mainFleet, escortFleet, enemyFleet, enemyEscort, packet.api_opening_atack, StageType.Opening)); // Shelling (Main), 1st

        stages.push(simulateShelling(mainFleet, escortFleet, enemyFleet, enemyEscort, packet.api_hougeki1)); // Shelling (Main), 2nd

        stages.push(simulateShelling(mainFleet, escortFleet, enemyFleet, enemyEscort, packet.api_hougeki2)); // Closing Torpedo Salvo
      ...
      if (enemyType === 1) {
        ...
        stages.push(simulateOpeningTorpedo(mainFleet, escortFleet, enemyFleet, enemyEscort, packet.api_opening_atack, StageType.Opening)); // Shelling (Escort)

        stages.push(simulateShelling(mainFleet, escortFleet, enemyFleet, enemyEscort, packet.api_hougeki1)); // Closing Torpedo Salvo

        stages.push(simulateTorpedo(mainFleet, escortFleet, enemyFleet, enemyEscort, packet.api_raigeki)); // Shelling (Any), 1st

        stages.push(simulateShelling(mainFleet, escortFleet, enemyFleet, enemyEscort, packet.api_hougeki2)); // Shelling (Any), 2nd
    ...
    if (fleetType === 2) {
      if (enemyType === 0) {
        ...
        stages.push(simulateShelling(mainFleet, escortFleet, enemyFleet, enemyEscort, packet.api_hougeki1, StageType.Main)); // Shelling (Main), 2nd

        stages.push(simulateShelling(mainFleet, escortFleet, enemyFleet, enemyEscort, packet.api_hougeki2, StageType.Main)); // Shelling (Escort)

        stages.push(simulateShelling(mainFleet, escortFleet, enemyFleet, enemyEscort, packet.api_hougeki3, StageType.Escort)); // Closing Torpedo Salvo
    ...
    if (fleetType === 1 || fleetType === 3) {
      if (enemyType === 0) {
        ...
        stages.push(simulateShelling(mainFleet, escortFleet, enemyFleet, enemyEscort, packet.api_hougeki1, StageType.Escort)); // Closing Torpedo Salvo

        stages.push(simulateTorpedo(mainFleet, escortFleet, enemyFleet, enemyEscort, packet.api_raigeki)); // Shelling (Main), 1st

        stages.push(simulateShelling(mainFleet, escortFleet, enemyFleet, enemyEscort, packet.api_hougeki2, StageType.Main)); // Shelling (Main), 2nd

        stages.push(simulateShelling(mainFleet, escortFleet, enemyFleet, enemyEscort, packet.api_hougeki3, StageType.Main));
      }
      ...
      if (enemyType === 1) {
        ...
        stages.push(simulateShelling(mainFleet, escortFleet, enemyFleet, enemyEscort, packet.api_hougeki1, StageType.Main)); // Shelling (Escort)

        stages.push(simulateShelling(mainFleet, escortFleet, enemyFleet, enemyEscort, packet.api_hougeki2, StageType.Escort)); // Closing Torpedo Salvo

        stages.push(simulateTorpedo(mainFleet, escortFleet, enemyFleet, enemyEscort, packet.api_raigeki)); // Shelling (Main), 2nd
  ...
  prcsNight(packet, path) {
    const {
      ...
      enemyFleet,
      enemyEscort
    } = this; // Night to Day Support

    stages.push(simulateSupport(enemyFleet, enemyEscort, packet.api_n_support_info, packet.api_n_support_flag)); // Night to Day Combat

    stages.push(simulateShelling(mainFleet, escortFleet, enemyFleet, enemyEscort, packet.api_n_hougeki1, StageType.Night));
    stages.push(simulateShelling(mainFleet, escortFleet, enemyFleet, enemyEscort, packet.api_n_hougeki2, StageType.Night)); // NPC Friend Support

    stages.push(simulateShelling(friendFleet, null, enemyFleet, enemyEscort, (packet.api_friendly_battle || {}).api_hougeki, StageType.Night)); // Night Combat
  ...
  get result() {
    if (this._result != null) return this._result; // Simulate battle result

    const rank = this._isAirRaid ? simulateAirRaidBattleRank(this.mainFleet, this.escortFleet) : simulateBattleRank(this.mainFleet, this.escortFleet, this.enemyFleet, this.enemyEscort);
    const mvp = this._isNightOnlyMVP ? [0, simulateFleetNightMVP(this.stages)] : [simulateFleetMVP(this.mainFleet), simulateFleetMVP(this.escortFleet)];
    return new Result({
      rank,
      mvp
    });
